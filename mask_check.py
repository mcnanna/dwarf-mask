...
"""
### Using individual masks ###

ebv_mask = ugali.utils.healpix.read_map('masks/healpix_mask_ebv.fits', nest=True)
cut_ebv = ebv_mask[pix]
cut_clean = cut_ebv & (d['DEC'] > -25.)

cut_modulus = (d['MODULUS'] < 21.75)

Nilson_mask = ugali.utils.healpix.read_map('masks/healpix_mask_Nilson.fits', nest=True)
cut_Nilson = Nilson_mask[pix]

ext_mask = ugali.utils.healpix.read_map('masks/healpix_mask_ext.fits', nest=True)
cut_ext = ext_mask[pix]
cut_associate = cut_ext

bsc_mask = ugali.utils.healpix.read_map('masks/healpix_mask_bsc.fits', nest=True)
cut_bsc = bsc_mask[pix]
...
pylab.hist(d['SIG'][cut_clean], bins=bins, color='blue', histtype='step', cumulative=-1, label='E(B-V) < 0.2 mag\n& Dec > -25 deg')
pylab.hist(d['SIG'][cut_clean & cut_modulus], bins=bins, color='green', histtype='step', cumulative=-1, label='above & (m - M) < 21.75') # 23.25
pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_Nilson], bins=bins, color='magenta', histtype='step', cumulative=-1, label='above & no Nilson73 association') # 23.25
pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_associate], bins=bins, color='purple', histtype='step', cumulative=-1, label='above & no galaxy association') # 23.25
pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_associate & cut_bsc], bins=bins, color='black', histtype='step', cumulative=-1, label='above & no BSC association') # 23.25
pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_associate & cut_bsc & cut_core], bins=bins, color='orange', histtype='step', cumulative=-1, label='above & ratio > 1.') # 23.25
pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_associate & cut_bsc & cut_core & cut_size], bins=bins, color='cyan', histtype='step', cumulative=-1, label='above & r > 0.025') # 23.25
#pylab.hist(d['SIG'][cut_clean & cut_modulus & cut_associate & cut_bsc], bins=bins, color='cyan', histtype='step', cumulative=-1, label='above & no BSC association') # 23.25
pylab.legend(loc='upper right')
pylab.xlabel('SIG')
pylab.ylabel('Cumulative Counts')
pylab.savefig('significance_distribution_mask.png')
"""

### Using combined masks ###
...
ebv_Nilson_mask = ugali.utils.healpix.read_map('masks/healpix_mask_ebv_Nilson.fits', nest=True)
cut_ebv_Nilson = ebv_Nilson_mask[pix]

ebv_ext_mask = ugali.utils.healpix.read_map('masks/healpix_mask_ebv_ext.fits', nest=True)
cut_ebv_associate = ebv_ext_mask[pix]

ebv_ext_bsc_mask = ugali.utils.healpix.read_map('masks/healpix_mask_ebv_ext_bsc.fits', nest=True)
cut_ebv_associate_bsc = ebv_ext_bsc_mask[pix]
...
pylab.hist(d['SIG'][cut_ebv_Nilson & cut_dec & cut_modulus], bins=bins, color='purple', histtype='step', cumulative=-1, label='above & no Nilson73 association') # 23.25
pylab.hist(d['SIG'][cut_ebv_associate & cut_dec & cut_modulus], bins=bins, color='magenta', histtype='step', cumulative=-1, label='above & no galaxy association') # 23.25
pylab.hist(d['SIG'][cut_ebv_associate_bsc & cut_dec & cut_modulus], bins=bins, color='black', histtype='step', cumulative=-1, label='above & no BSC association') # 23.25
pylab.hist(d['SIG'][cut_ebv_associate_bsc & cut_dec & cut_modulus & cut_core], bins=bins, color='orange', histtype='step', cumulative=-1, label='above & ratio > 1.') # 23.25
pylab.hist(d['SIG'][cut_ebv_associate_bsc & cut_dec & cut_modulus & cut_core & cut_size], bins=bins, color='cyan', histtype='step', cumulative=-1, label='above & r > 0.025') # 23.25
pylab.legend(loc='upper right')
pylab.xlabel('SIG')
pylab.ylabel('Cumulative Counts')
...
healpy.projscatter(d['RA'][cut & cut_ebv_associate & cut_dec & cut_modulus], d['DEC'][cut & cut_ebv_associate & cut_dec & cut_modulus], lonlat=True, c='magenta', marker='o', edgecolor='none', s=10, vmax=0.5)
...
cut_sig = (d['SIG'] > 6.)
cut_final = cut_ebv_associate_bsc & cut_dec & cut_modulus & cut_core & cut_size & cut_sig


###############################################################
# Below is all stuff that was included in Keith's original code. 
# Since I'm posting this here mostly so Keith can look at it, I'm
