...
def sighist(cands, ax, legend=True, title=True, text=True):
    if cands.alg == 'simple':
        bins = np.arange(5., 40.25, 0.5)
    elif cands.alg == 'ugali':
        bins = np.logspace(np.log10(10.), np.log10(200000.), num=70)
        ax.set_xscale('log')
    ax.set_ylim(10**0, 3*10**4)
    ax.set_yscale('log')
    ax.hist(cands.data[cands.SIG], bins=bins, color='red', histtype='step', cumulative=-1, label='All')
    ax.hist(cands.data[cands.SIG][cands.cut_ebv & cands.cut_footprint], bins=bins, color='blue', histtype='step', cumulative=-1, label= r'$E(B-V) < 0.2$ mag & in footprint')
    ax.hist(cands.data[cands.SIG][cands.cut_ebv & cands.cut_footprint & cands.cut_modulus], bins=bins, color='green', histtype='step', cumulative=-1, label=r'above & $m - M < {}$'.format(21.75 if cands.survey == 'ps1' else 23.5))
    ...
    ax.hist(cands.data[cands.SIG][cands.cut_ebv & cands.cut_footprint & cands.cut_modulus & cands.cut_associate & cands.cut_bsc], bins=bins, color='black', histtype='step', cumulative=-1, label='above & no catalog assoc.') # = cands.cut_bulk
    ...
    if cands.cross:
        ax.hist(cands.data[cands.SIG][cands.cut_bulk & cands.cut_dwarfs & cands.cut_cross], bins=bins, color='darkturquoise', histtype='step', cumulative=-1, label='above & found by both algorithms') 
    if legend:
    ...
    if title:
        ...
        ax.set_title(' ')
    #ax.title('{}'.format('Pan-STARRS' if cands.survey == 'ps1' else 'DES', 'SimpleBinner' if cands.alg == 'simple' else 'Ugali'))
    if text:
        ax.text(0.9, 0.55, cands.survey.upper(), transform=ax.transAxes, horizontalalignment='left', fontsize=12, weight='bold')
    #ax.set_xlabel(cands.SIG)
    #ax.set_ylabel('Cumulative Count')

fig, axs = pylab.subplots(2, 2)
sighist(des_simple, axs[0,0], title=True, text=False, legend=False)
sighist(des_ugali, axs[0,1], title=True, text=True, legend=False)
sighist(ps1_simple, axs[1,0], title=False, text=False, legend=False)
sighist(ps1_ugali, axs[1,1], title=False, text=True, legend=False)
"""
